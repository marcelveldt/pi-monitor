...
from resources.lib.utils import LOGGER, try_decode, try_encode, etree_to_dict
...
class Infos(object):
	...
	def __init__(self):
		...
		self.playstate = self.STOPPED				# Enum: Infos.PLAYING, Infos.STOPPED
...
class CoverArt(object):
	...
	def base64(self):
		if self._base64:
			return self._base64
		if self._binary:
			self._base64 = encodebytes(try_encode(self._binary))
			return self._base64
		else:
			return None
...
class Item(object):
	...
	@property
	def data_str(self):
		return try_decode(self.data)

	@property
	def data_int(self):
		if self.data:
			try:
				return int("0x" + ''.join([hex(ord(x))[2:] for x in self.data]), base=16) #py2
			except:
				return int("0x" + ''.join([hex(x)[2:] for x in self.data]), base=16) #py3
	...
	def data_base64(self):
		if self._data_base64:
			...
			return encodebytes(try_encode(self.data))
#end def
